...
enum KireicchiWidgetConstants {
    ...
    /// Widget Extension に渡す部屋画像の長辺上限（px）。
    /// フル解像度（1024x1024）のままレンダリングすると Widget Extension の
    /// メモリ上限を超えプロセスが強制終了されるため、共有前にここまで縮小する。
    static let maxSharedImageDimension: CGFloat = 300
}
